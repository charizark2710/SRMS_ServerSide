...
export class ReportErrorController {
    ...
    init() {
        this.router.post(this.path + "/sendReportError", this.sendRepportError);
        // this.router.post(this.path, this.createBookingRoom);
        this.router.delete(this.path + "/delete/:id", this.deleteReportError);
        this.router.patch(this.path + "/acceptOrRejectReportError", this.acceptOrRejectReportError);
        // this.router.put(this.path + "/updating", this.updateBooking);
        this.router.get(this.path + '/:id', this.viewDetailReportError);
    ...
    deleteReportError = async (request: express.Request, response: express.Response) => {
        try {
            ...
            var userId = reportErrId?.split('_')[0] || ' ';
            //xóa trong booking, xóa hết noti
            await db.ref('ReportError').child(reportErrId).remove()
                .then(function () {
                    //xoa noti user
                    db.ref('notification').child(userId).child(reportErrId.toString()).remove();
                    //xoa noti admin
                    db.ref('notification').child('thanhntse63563').child(reportErrId.toString()).remove();
                })
                .catch(function (error) {
