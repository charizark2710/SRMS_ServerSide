...
export class RequestListController {
    ...
    init() {
        ...
        this.router.get(this.path, this.getRequestList);
        this.router.delete(this.path + '/delete/:ids', this.deleteRequestList);
    }

    getRequestList = async (request: express.Request, response: express.Response) => {
        try {
            var result: any[] = [];
            (await db.ref('notification').child('thanhntse63563').get()).forEach(snapshot => {
    ...
    deleteRequestList = async (request: express.Request, response: express.Response) => {
        try {
            const deleteRequest = request.params.ids;
            let deleteRequestIds:string[] = deleteRequest?.split(',');
            if (deleteRequestIds){
                for (let index = 0; index < (deleteRequestIds.length-1); index++) {
                    await db.ref('notification').child('thanhntse63563').child(deleteRequestIds[index]).remove();
                }
            }
